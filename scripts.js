...
let arrayFilmes = localStorage.getItem("listaFilmes")
  ? JSON.parse(localStorage.getItem("listaFilmes"))
  : [];

let arraySeries = localStorage.getItem("listaSeries")
  ? JSON.parse(localStorage.getItem("listaSeries"))
  : [];

let arrayAssistidos = localStorage.getItem("listaAssistidos")
  ? JSON.parse(localStorage.getItem("listaAssistidos"))
  : [];

let listaPrincipal = new ListaFS(
  new Map(arrayFilmes),
  new Map(arraySeries),
  new Map(arrayAssistidos)
);
...
pesquisaFilme.addEventListener("input", (e) => {
  mostrarResultadosPesquisa(e, "movie");
});
pesquisaSerie.addEventListener("input", (e) => {
  mostrarResultadosPesquisa(e, "tv");
});
...
const getAPI = async (tipo, nome) => {
  try {
    let urlMontada = `${urlAPI}${tipo}?api_key=${keyAPI}&query=${encodeURIComponent(
      nome
    )}&language=pt-BR`;
...
async function mostrarResultadosPesquisa(e, tipo) {
  if (e.target.value.trim() !== "") {
    btnAddTitulo.style.display = "none";
    try {
      const dados = await getAPI(tipo, e.target.value);
...
function addTitulo() {
  if (tituloSelecionado.title) {
    if (
      listaPrincipal.listaFilme.has(tituloSelecionado.id) ||
      listaPrincipal.listaAssistido.has(tituloSelecionado.id)
    ) {
      alert("Esse filme já foi adicionado ou assistido.");
    } else {
      listaPrincipal.addTitulo(
        conteinerFilmes,
        listaPrincipal.listaFilme,
        tituloSelecionado
      );
      fecharModal();
      conteinerFilmes.style.display = "flex";
    ...
    if (
      listaPrincipal.listaSerie.has(tituloSelecionado.id) ||
      listaPrincipal.listaAssistido.has(tituloSelecionado.id)
    ) {
...
function lerDados() {
  ...
  listaPrincipal.mostrarTitulos(
    conteinerAssistidos,
    listaPrincipal.listaAssistido
  );
}
